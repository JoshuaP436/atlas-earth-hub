...
export default function StrategyPage() {
  ...
  const generateStrategies = (): Strategy[] => {
    ...
    // Strategy 1: All Common Parcels (No Badges)
    const commonCount = Math.floor(budget / PARCEL_DATA.common.cost);
    if (commonCount > 0) {
      const baseRent = PARCEL_DATA.common.rent * commonCount;
      const dailyReturn = calculateDailyEarnings(baseRent, 0, selectedAdBoost, boostHoursPerDay);
      const investment = commonCount * PARCEL_DATA.common.cost;
      
      newStrategies.push({
        name: `${commonCount}x Common Parcels Only`,
        investment,
        dailyReturn,
        breakEven: investment / dailyReturn,
        efficiency: dailyReturn / investment,
        description: `Maximum parcels strategy - ${commonCount} Commons with ${boostHoursPerDay}h of ${selectedAdBoost}x ad boosts daily`
      });
    }

    // Strategy 2: Badge Investment + Common Parcels 
    if (budget >= BADGE_SYSTEM.cost) {
      const badgeRec = getBadgeRecommendation(budget);
      const remainingForParcels = badgeRec.remainingBudget || (budget - badgeRec.cost);
      const commonCountWithBadges = Math.floor(remainingForParcels / PARCEL_DATA.common.cost);
      
      if (commonCountWithBadges > 0 && badgeRec.badges > 0) {
        const baseRent = PARCEL_DATA.common.rent * commonCountWithBadges;
        const dailyReturn = calculateDailyEarnings(baseRent, badgeRec.bonus, selectedAdBoost, boostHoursPerDay);
        const totalInvestment = badgeRec.cost + (commonCountWithBadges * PARCEL_DATA.common.cost);
        
        newStrategies.push({
          name: `${badgeRec.badges} Badges + ${commonCountWithBadges}x Common`,
          investment: totalInvestment,
          dailyReturn,
          breakEven: totalInvestment / dailyReturn,
          efficiency: dailyReturn / totalInvestment,
          description: `${badgeRec.tier} permanent rent bonus + ${commonCountWithBadges} Commons`
        });
      }
    }
